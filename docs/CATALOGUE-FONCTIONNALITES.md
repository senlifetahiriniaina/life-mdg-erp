...
- **Cycle acompte / solde sur les commandes** : demande et encaissement d'un acompte à la commande, puis facturation et encaissement du solde à la livraison — côté commandes clients comme côté commandes fournisseurs, avec génération automatique d'une facture liée et d'une écriture comptable équilibrée à chaque étape, et un indicateur d'avancement (acompte demandé/payé, solde facturé/payé) visible directement sur la commande.
- **Prévisions de trésorerie** : projection journalière des encaissements et décaissements attendus (30/60/90/180 jours) construite sur l'historique réel des mouvements comptables, avec détection automatique des périodes à risque de déficit, narration en langage clair, et export PDF/Excel.
- **Budget généré depuis l'historique et corrélé aux objectifs commerciaux** : construction automatique d'un budget de dépenses et de revenus à partir de la moyenne réelle des derniers mois, avec les lignes de revenu recalées sur les objectifs commerciaux validés lorsqu'ils couvrent la période budgétée — plutôt que de partir d'une feuille blanche.
- **Revue finance périodique (mensuelle/trimestrielle)** : un écran dédié compare, sur la période choisie, la réalisation des objectifs commerciaux et du budget par rapport au réel (grand livre comptable), avec un code couleur de performance et un espace de commentaires pour formaliser la revue et en garder l'historique.
...
- **Commandes récurrentes** : pour un client répétitif (le même article, à échéance régulière), un modèle réutilisable (client + lignes + périodicité hebdomadaire/mensuelle/trimestrielle) génère automatiquement une vraie commande à chaque échéance, sans ressaisie — avec une génération manuelle possible à tout moment en plus de la génération automatique planifiée.
- **Objectifs commerciaux assistés par IA** : l'application propose trois cibles chiffrées (conservatrice, modérée, ambitieuse), calculées mathématiquement à partir de l'historique réel des ventes des 6 derniers mois — jamais inventées par l'IA — au niveau global, par commercial, par client ou par catégorie de produits ; l'IA ne fait que rédiger la justification de chaque proposition, que l'utilisateur peut ensuite ajuster et valider.
...
- **Suivi de la sous-traitance de production** : un pipeline de statuts simple (matières réunies → en sous-traitance → contrôle qualité → prêt à livrer → livré) permet de savoir en permanence où en est un article confié à un façonnier, avec délai et respect des échéances suivis par sous-traitant.
- **Traçabilité bout-en-bout d'une commande sur mesure** : un seul écran relie la fiche de chiffrage initiale, la commande client (avec son avancement d'acompte/solde), les commandes d'achat de matières associées et l'état de la sous-traitance — pour retrouver en un coup d'œil l'historique complet d'un article, du devis à la livraison.
- **Bibliothèque de templates de produits éditable** : les modèles de produits par famille (matières premières, accessoires, produits semi-finis, produits finis) sont entièrement gérables depuis l'interface — création, modification, désactivation — pas seulement une liste figée fournie par défaut.
