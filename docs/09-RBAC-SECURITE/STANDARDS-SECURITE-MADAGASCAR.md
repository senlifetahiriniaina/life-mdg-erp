...
### 3.9 Validation des entrées — Réel (avec une note de nommage trompeur, historique)

`Modules/Validation` **n'est pas** un framework de validation d'entrées malgré son nom — c'est un moteur de workflow d'approbation (`ApprovalRequest`/`ApprovalWorkflow`/`ApprovalRule`/`ApprovalHierarchy`/etc.), sans rapport avec la validation d'entrée. La validation d'entrée réelle se fait de façon standard via des classes `FormRequest` par module — correct, pas une lacune de sécurité en soi.

Ce moteur d'approbation a été réparé en profondeur dans une itération ultérieure (schéma jusque-là stub sur 4 tables, `eval()` sur une chaîne stockée en base remplacé par un `match()` sur un enum `condition_operator` validé à l'écriture, résolution des approbateurs par rôle en plus de `user_id` fixe, prise en compte des congés/horaires de travail avec délégation automatique au suppléant puis escalade hiérarchique). Accounting (factures) et Achats (bons de commande) routent désormais leurs approbations dessus au lieu d'implémentations bespoke disjointes. Journal des décisions réel via `Modules\Core\Traits\RecordsActivity` (`ApprovalAction`/`ApprovalHistory`).

**Mini-WAF applicatif — câblé.** Les services XSS/encodage de sortie (`XssPreventionService`, `OutputEncodingService`, `HtmlPurifierService`) avaient une vraie logique mais zéro appelant dans le pipeline requête/réponse — code mort. De même, `Modules\Security\Models\ThreatIndicator` n'avait pas de `$table` explicite et pointait par défaut sur `threat_indicators`, une table qui n'a jamais existé (la vraie table créée par migration est `security_threat_indicators`) — `ThreatDetectionService::isKnownThreatIp()` n'avait donc jamais pu s'exécuter avec succès. Les deux sont maintenant corrigés et branchés via `App\Http\Middleware\RequestInspectionMiddleware`, enregistré globalement (web + api, juste après `SecurityHeaders`) :
- Vérifie l'IP contre `ThreatIndicator` (résultat mis en cache 5 min, pas de requête DB par requête).
- Scanne récursivement les champs texte de la requête via `XssPreventionService::detectXss()` (détection pure, pas de réécriture silencieuse du contenu).
- **Mode shadow par défaut** (`REQUEST_INSPECTION_SHADOW_MODE=true` / `config('security.request_inspection.shadow_mode')`) : logue les correspondances sur le canal `security` (`storage/logs/security-*.log`) sans jamais bloquer — les patterns XSS génériques peuvent faire des faux positifs sur des champs texte libre légitimes (notes RH, descriptions CRM, tickets Helpdesk) et ce middleware n'avait jamais tourné sur du trafic réel. Passer en mode blocage (403/422) seulement après une période d'observation des logs, en configurant `security.request_inspection.excluded_fields`/`excluded_routes` pour les champs texte-riche connus.
- Portée volontairement XSS + réputation IP uniquement pour cette passe — pas de détection SQLi (l'app utilise Eloquent/query builder partout, risque structurellement plus faible ; ajouter une détection SQLi hâtive en même temps que le premier branchement réel de ces services augmenterait le risque de faux positifs). Suivi séparé, pas glissé sous silence.
- Ne fait pas doublon avec le rate limiting existant (`throttleApi`/limiteurs nommés, §3.6) — concept disjoint : volume de requêtes vs contenu/réputation IP.
...
| Passer `RequestInspectionMiddleware` du mode shadow au mode blocage (§3.9) | Nécessite une période d'observation réelle des logs `security` pour calibrer `excluded_fields`/`excluded_routes` et éviter de bloquer du trafic légitime |
| `SecurityHeadersService` dupliqué (`Modules/Core/app/Services/`) toujours jamais appelé | Cosmétique, sans risque — `App\Http\Middleware\SecurityHeaders` (§3.8) est la version réellement branchée |
