...
|---|---|---|
| `Ticket` | `hd_tickets` | Ticket de support (sujet, statut, priorité, canal, SLA, satisfaction, `company_id` — ajouté Chantier 32.21, voir Particularités) ; relation polymorphe `source()` vers l'enregistrement d'origine dans un autre module |
...
| `SentimentScore`, `LanguageDetection`, `EscalationPrediction`, `SatisfactionPrediction`, `AgentMetric`, etc. (préfixe `cs_*`) | `cs_*` | Modèles d'analyse/scoring IA (sentiment, prédiction d'escalade et de satisfaction, performance agent) — `SentimentScore`/`LanguageDetection` sont désormais réellement peuplés à chaque création de ticket (Chantier 32.21, voir Particularités) ; nombreux autres modèles annexes non détaillés ici |
...
| Auth + `module:Helpdesk` | GET/POST/PUT/DELETE | `helpdesk/tickets`, `helpdesk/tickets/{ticket}` | CRUD ticket ; `source_module`/`source_id` optionnels à la création (voir Particularités) ; `index()` scopé par `company_id` (Chantier 32.21, nul-safe des deux côtés) |
| — | POST | `helpdesk/tickets/{ticket}/assign\|auto-assign\|resolve\|close\|escalate` | Cycle de vie du ticket — `assign`/`resolve`/`escalate` avaient **zéro** `authorize()` avant Chantier 32.21 (trou RBAC réel, corrigé) ; `auto-assign` (nouveau) délègue à `TicketAssignmentService::assignRoundRobin()`, jusque-là un service réel sans aucun appelant |
...
- **`TicketAssignmentService`** — assignation en file d'attente par répartition round-robin ; réellement branché sur un vrai appelant depuis Chantier 32.21 (`POST tickets/{ticket}/auto-assign`), jusque-là un service testé mais sans aucun consommateur réel.
...
- **`SentimentAnalysisService`** / **`SatisfactionPredictionService`** — analyse de sentiment multilingue et prédiction ML de la satisfaction client. `SentimentAnalysisService::analyzeTicketSentiment()` s'exécute désormais automatiquement à chaque création de ticket (`TicketObserver::recordSentimentAndLanguage()`, Chantier 32.21) — auparavant sans aucun producteur réel malgré `CustomerServiceAIController::getSentimentAnalysis()`/`getLanguageDetection()` qui ne faisaient jamais que *lire* `SentimentScore`/`LanguageDetection`, deux modèles jamais écrits nulle part.
- **`AgentPerformanceAnalyticsService`** — métriques de performance agent (temps de résolution, FCR, CSAT).

**`AlertService` supprimé (Chantier 32.21)** — confirmé doublon mort de `SlaAutomationService`/`SlaService` (mêmes notifications SLA, jamais réellement câblé nulle part hors de deux appels internes à `TicketAssignmentService`, eux-mêmes retirés) — voir Particularités.
...
**Trou RBAC corrigé (Chantier 8.2)** : `CustomerServiceAIPolicy` (46 méthodes d'habileté, couvrant les 21 endpoints de `CustomerServiceAIController`) existait, était bien écrite, mais n'était **jamais appelée depuis le contrôleur et jamais enregistrée auprès du Gate de Laravel** — n'ayant pas de modèle Eloquent unique associé, ni l'auto-découverte ni `AppServiceProvider::$policies` ne pouvaient la trouver. N'importe quel utilisateur authentifié du module Helpdesk pouvait donc atteindre les 21 endpoints sans aucune vérification. Corrigé par : (1) `HelpdeskServiceProvider::boot()` enregistre désormais chaque méthode publique de la policy comme sa propre habileté Gate (`Gate::define($ability, [CustomerServiceAIPolicy::class, $ability])`, en bouclant sur `get_class_methods()`) ; (2) `$this->authorize()` ajouté aux 21 méthodes du contrôleur ; (3) les 44 chaînes de permission `helpdesk.*` distinctes que la policy vérifie sont désormais seedées (`HELPDESK_EXTRA_PERMISSIONS`, nouveau bloc du seeder) — `support-admin`/`customer-service`/`admin` les couvrent déjà via leur correspondance générique sur le préfixe `helpdesk.`, aucun autre changement de seeder n'était nécessaire.

**Trou RBAC corrigé (Chantier 32.21, audit approfondi en 14 couches)** : `hd_tickets` n'avait **aucune colonne `company_id`** — `TicketController::index()`/`TicketPolicy`'s admin/manager/supervisor bypass n'avaient donc aucun cloisonnement multi-tenant, un trou de cloisonnement réel confirmé empiriquement. Corrigé par une migration additive (`company_id` nullable/indexé, backfillé depuis le rapporteur du ticket), `TicketService::createFromSource()` peuplant `company_id` depuis l'utilisateur authentifié à la création, et une nouvelle méthode `TicketPolicy::sameCompany()` (nul-safe des deux côtés — un ticket ou un utilisateur sans `company_id` reste permissif, seul un vrai conflit entre deux sociétés différentes bloque). Séparément, `TicketController::assign()`/`resolve()`/`escalate()` avaient **zéro** appel `authorize()` du tout (seul `update()`/`close()`/`delete()` étaient gardés) — n'importe quel utilisateur authentifié du module pouvait réassigner/résoudre/escalader n'importe quel ticket, confirmé empiriquement. Corrigé avec les habiletés `assignTicket`/`closeTicket`/`update` déjà existantes sur `TicketPolicy`, jusque-là écrites mais jamais appelées pour ces 3 actions précises.
...
- **`Modules\AI`** : `HelpdeskAiAssistController` appelle `AiContextualAssistantService`. Le module `'Helpdesk'` (`supportedModules()`) couvre désormais 12 actions (Chantier 32.21, contre 2 avant) — 8 des pages Vue réelles du module n'appelaient jamais `useAiAssistant()` (`Tickets/Index`, `Chat/Index`, `Portal/Index`, `Escalation/Index`, `KnowledgeBase/Index`, `Forum/Index`+`Show`, `CSAT/Index`, `SlaAutomation/Index`, `AIBot/Index`, `QualityAssurance/Index`) — câblées.
...
**8 tables manquantes ajoutées (Chantier 8.2)** (`ConversationAnalytics`, `EscalationHistory`, `EscalationWorkflow`, `ResponseCustomization`, `ResponsePerformance`, `SatisfactionFactor`, `SatisfactionHistory`, `SatisfactionModel`, préfixe `cs_*`) : ces 8 modèles avaient un `$fillable`/`$casts` corrects mais aucune table du tout. **Réaudité au Chantier 32.21 (couche 9, fake/dead)** : 4 des 8 (`ConversationAnalytics`, `EscalationWorkflow`, `ResponseCustomization`, `SatisfactionFactor`) confirmés totalement isolés — zéro relation, zéro contrôleur/route, zéro appelant réel nulle part (grep exhaustif) — supprimés avec leur factory et leurs 2 habiletés `CustomerServiceAIPolicy` mortes associées (`manageEscalationWorkflows()`/`manageSatisfactionFactors()`). Les 4 autres (`EscalationHistory`, `ResponsePerformance`, `SatisfactionHistory`, `SatisfactionModel`) restent dormants mais **conservés** — écrits pour un usage futur documenté, pas de duplication confirmée d'un autre modèle réel, donc pas de décision de suppression tranchée à ce stade.

## Chantier 32.21 — audit approfondi en 14 couches

- **Sécurité approfondie (couche 6)** — cloisonnement multi-tenant sur `hd_tickets` et 3 endpoints sans `authorize()` du tout, voir RBAC ci-dessus.
- **Fake/Dead (couche 9)** — `AlertService` (doublon mort de `SlaAutomationService`/`SlaService`) supprimé ; `TicketAssignmentService::assignRoundRobin()` (réel, testé, zéro appelant) activé via un nouvel endpoint `POST tickets/{ticket}/auto-assign` ; `SentimentAnalysisService` (réel, déterministe, zéro producteur) activé via `TicketObserver::recordSentimentAndLanguage()` sur chaque création de ticket ; `EmotionAnalysis` délibérément laissé non câblé — son jeu fixe de 6 émotions (anger/frustration/satisfaction/confusion/urgency/disappointment) ne correspond pas au jeu différent de `SentimentAnalysisService::EMOTIONS` (anger/frustration/satisfaction/confusion/gratitude/fear), une vraie décision de conception à trancher, pas une simple omission de câblage.
- **IA (couche 13)** — voir Dépendances ci-dessus : 8 pages réelles jamais câblées à `useAiAssistant()`.
- **Import/Export/rapport (couche 14c)** — `AgentPerformanceController::reportExport()`/`metricsExport()` retournaient auparavant une URL fabriquée pointant vers un fichier jamais réellement généré (aucun appel à une librairie PDF/Excel nulle part dans la classe) — un 404 garanti sur chaque tentative réelle de téléchargement. Les deux génèrent désormais un vrai fichier (DomPDF pour `reportExport()` format PDF, `Maatwebsite\Excel` pour les deux en XLSX — `Modules/Helpdesk/app/Exports/{AgentPerformanceReportExport,AgentMetricsExport}.php` + `Modules/Helpdesk/resources/views/exports/agent-performance-report.blade.php`), enregistré sur le disque `public`, avec une vraie `export_url` fonctionnelle — l'enveloppe JSON existante (`getJson()`+`export_url`) est conservée pour ne pas casser le contrat déjà testé. Un bug trouvé en construisant le correctif : `metricsExport()` interrogeait `User::role('support-agent')` — la portée `role()` de Spatie lève `RoleDoesNotExist` (une vraie erreur 500) dès que ce rôle n'a jamais été semé du tout, pas seulement quand personne ne le porte — corrigé en interrogeant directement le pivot (`whereHas('roles', ...)`), qui dégrade proprement vers une collection vide.
- **Confirmé, non touché (déjà correctement câblé)** : `helpdesk:check-sla-breaches` apparaît bien dans `php artisan schedule:list` (correctif Chantier 19/root-scheduler-fix toujours en place) ; le morph-map `HelpdeskLinkable` (Chantier 19) tient toujours ; les 44 permissions `CustomerServiceAIPolicy` (Chantier 8.2) restent seedées et fonctionnelles.
