...
- `Services\AI\AIService` (+ `AnthropicProvider`, `OpenAIProvider`, `DeepSeekProvider`) — service IA central, provider-agnostic via `AIProviderContract` (`chat()`, `embed()`, `analyze()`, `getProviderName()`, `isConfigured()`). Le provider actif est résolu par `AI_DEFAULT_PROVIDER` (`config('ai.default_provider')`, `anthropic` par défaut) ou par un override `module_providers` ciblé par module. En plus d'Anthropic/OpenAI, un troisième provider auto-hébergé `DeepSeekProvider` (servi via Ollama, voir `docker-compose.deepseek.yml` et `docs/07-DEPLOIEMENT/IA-AUTOHEBERGEE.md`) est disponible, activable sans changer le code appelant. Exposé via `POST /api/v1/ai/ask|analyze|generate-document` et consommé par `Modules\AI\Services\AiContextualAssistantService` (voir Particularités — ce n'est plus une pile IA isolée du Core).
...
- Core **n'importe pas** `Modules\AI\Services\AiContextualAssistantService` — la dépendance va dans l'autre sens : le module `AI` importe `Modules\Core\Services\AI\AIService` (voir ci-dessous).
...
- **Une seule pile IA, deux points d'entrée** : Core expose `Services\AI\AIService` (`AIProviderContract` : `AnthropicProvider`/`OpenAIProvider`/`DeepSeekProvider`) via `POST /api/v1/ai/ask|analyze|generate-document`. Le module `AI` (`AiContextualAssistantService::getGuidance()`, utilisé par 33 autres fichiers du codebase pour le pattern « AI Assisted First ») délègue à cette même instance (`AIService::forModule('AI')`) plutôt que d'avoir sa propre logique d'appel HTTP — les deux partagent désormais le même provider actif, la même config (`config/ai.php`), et donc le même comportement de repli en cas d'échec.
