...
**Vulnérabilité cross-tenant corrigée en deux temps** : le helper `tenantId()` partagé de `SetupController`/`OnboardingMetricsController` résolvait à l'origine `$request->user()?->company_id ?? $request->header('X-Company-ID') ?? 0` — `company_id` étant fréquemment `null` pour un utilisateur ordinaire, le repli sur l'en-tête client était atteint dans le cas courant, permettant à tout utilisateur authentifié de définir `X-Company-ID: <id-du-tenant-victime>` pour lire/écrire les jobs d'import et sessions d'onboarding d'un autre tenant. Une session précédente (8.5sv) avait corrigé cela en `$request->user()?->tenant_id ?? 0` (aucun repli sur en-tête), en s'appuyant sur le docblock de `App\Http\Middleware\InitializeTenancyFromAuthenticatedUser` pour conclure que `tenant_id` était « la vraie colonne de délimitation multi-tenant ». **Chantier 10 a réinvestigué et corrigé cette conclusion** : ce docblock décrit uniquement l'intention de conception du middleware (et son repli sûr quand `tenant_id` est absent) — il ne prouve pas que la colonne est réellement alimentée. Une recherche exhaustive sur tout le dépôt (contrôleur d'inscription `AuthController::register()`, `DemoSeeder`, toutes les factories) confirme que `users.tenant_id` n'est écrit **nulle part** dans un vrai chemin de création d'utilisateur — exactement le même champ fantôme déjà corrigé ailleurs cette session (Reporting, Strategy, AI, Sales, Achats, Integration, Workflow, Payroll). Le résultat pratique était identique au bug initial : toutes les entreprises voyaient leurs jobs d'import/sessions d'onboarding s'agréger silencieusement dans le même panier `tenant_id=0`. Corrigé vers `$request->user()?->company_id ?? 0` — toujours sans repli sur en-tête, donc la vulnérabilité IDOR d'origine reste fermée.
