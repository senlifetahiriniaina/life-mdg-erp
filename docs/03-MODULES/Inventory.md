...
- **`InventoryService`** — opérations cœur de gestion de stock (réception, ajustement, transfert). `StockManagementService` (scaffold `Cache`-only, sans persistance réelle) et `StockService`/`SKU` (sous-système « SKU × Entrepôt » parallèle, jamais intégré au vrai catalogue `Product`) ont été **supprimés au Chantier 32.22** — confirmés morts, zéro appelant réel nulle part hors leurs propres tests isolés.
- **`ReorderAutomationService`** — surveille le stock par tenant, calcule le point de commande (avec facteurs saisonniers Afrique de l'Ouest/Est/Madagascar : Ramadan, rentrée scolaire, récolte), sélectionne le fournisseur préféré et **crée directement un `Modules\Achats\Models\PurchaseOrder`** quand le stock passe sous le seuil — c'est le point d'entrée concret du couplage bidirectionnel Inventory ↔ Achats. **Confirmé toujours mort** (Chantier 17c, re-confirmé Chantier 32.22) : zéro contrôleur/route/commande ne l'invoque, laissé en l'état.
...
- **`AI\InventoryAIService`** (classification ABC par prompt LLM, détection d'anomalies/obsolescence, suggestions de réapprovisionnement) — **`AI\ABCAnalysisService`** (moteur déterministe Pareto/EOQ/point-de-commande/stock-de-sécurité, distinct et complémentaire du précédent) était réel, testé, mais sans aucun contrôleur/route — activé au Chantier 32.22 via `AbcAnalysisController` (`GET abc-analysis`, `GET abc-analysis/velocity`), après correction d'un bug fatal (`analyzeVelocity()`/`getAnnualDemand()` référençaient une classe `Modules\Inventory\Models\InventoryMovement` qui n'a jamais existé — corrigé vers le vrai `StockMovement`).
...
- **Strategy** (consommateur) : `KPIRegistryService::inventoryKPIs()` interroge `inventory_products`/`inventory_stock`/`inventory_stock_movements` directement — corrigé au Chantier 32.22 pour filtrer sur le vrai `company_id` plutôt que sur `tenant_id` (voir ce chantier ci-dessous).
...
## Chantier 32.22 — audit approfondi en 14 couches

Voir l'entrée `CLAUDE.md` correspondante pour le détail complet (bugs trouvés/corrigés, ce qui a été délibérément laissé en l'état, vérification). Résumé :

- **Layer 6 (sécurité approfondie)** : le gap déjà documenté (« zero company/tenant scoping module-wide », Chantier 19) était toujours réel, confirmé empiriquement avec 2 vraies sociétés et de vraies requêtes HTTP. Corrigé pour les 6 ressources les plus sensibles (`products`, `categories`, `warehouses`, `suppliers`, `stock-movements`, `purchase-orders`) via une nouvelle colonne `company_id` + un nouveau trait `ScopesToCompany` appliqué aux contrôleurs API **et** Web (`InventoryWebController::suppliers()`/`purchaseOrders()`, `Web\ProductController::index()`/`show()`/`edit()` — une fuite server-side, pas seulement API). `Unit`/`ProductTemplate` délibérément exclus (données de référence partagées, pas des données propres à une société). Les ~13 types de ressources opérationnelles restants (lots, picking, expéditions, RMA, comptages cycliques, prévisions de demande, valorisations, transferts inter-entrepôts, facteurs saisonniers, crossdock, picking par vagues, fiches de chiffrage, commandes de production) restent non cloisonnés — un gap de même sévérité que documenté ailleurs cette session pour CRM/HR, trop large pour ce seul passage.
- **2 vrais bugs de portée `orWhere`/opérateur trouvés en construisant le correctif ci-dessus** : `CategoryController::index()` utilisait `'ilike'` (opérateur Postgres-only, jamais utilisé par cette app — MySQL/SQLite seulement) — 500 garanti sur toute recherche réelle, jamais testé avant ; corrigé en `LIKE`. `WarehouseController::index()`/`InventoryWebController::suppliers()` avaient un `orWhere()` non parenthésé qui, combiné au nouveau scope `company_id`, aurait cassé le cloisonnement pour toute correspondance sur `city`/`code` — corrigé en regroupant dans une closure.
- **Layer 9 (fake/dead) — 3 décisions explicites** :
  1. **`ABCAnalysisService` — activé.** Moteur Pareto/EOQ/point-de-commande déterministe réel et testé, zéro route. `analyzeVelocity()`/`getAnnualDemand()` référençaient une classe `InventoryMovement` qui n'a jamais existé (fatal garanti si jamais appelées avec un metric ≠ `value`) — corrigé vers `StockMovement`, confirmé par exécution réelle. Routé (`GET abc-analysis`, `GET abc-analysis/velocity`).
  2. **`StockManagementService` — mort confirmé, supprimé.** Scaffold 100 % `Cache`, aucune persistance réelle, zéro consommateur — son propre commentaire de test préexistant l'avait déjà identifié comme factice sans jamais le supprimer.
  3. **`SKU`/`StockService` — mort confirmé, supprimé.** Sous-système « SKU × Entrepôt » parallèle jamais intégré au vrai `Product`/`Stock`/`StockMovement`, zéro appelant réel hors 3 fichiers de test racine isolés (`tests/Unit/Models/Inventory/SKUTest.php`, `tests/Unit/Services/Inventory/StockServiceTest.php`, `tests/Integration/Inventory/StockManagementTest.php` — le même angle mort déjà documenté au Chantier 32.13-32.16). Table `inventory_skus` et colonne fantôme `inventory_stock_movements.sku_id` supprimées par une nouvelle migration.
- **Layer 11 (CORE)** : `Product`/`Category`/`Warehouse` n'avaient aucune couverture `RecordsActivity` (audit trail) malgré Compliance First, contrairement aux modèles plus récents (Chantiers 17-24) — ajoutée aux 3 (`StockMovement` volontairement exclu, déjà son propre enregistrement d'audit par conception). Un vrai bug cross-module trouvé et corrigé : `Modules\Strategy\Services\KPIRegistryService::inventoryKPIs()` filtrait `inventory_products.tenant_id` — une colonne liée au mécanisme `BelongsToTenant`/stancl-tenancy propre à `Product`, jamais alimentée par le vrai chemin d'écriture (`ProductController::store()` écrit toujours `auth()->user()->tenant_id`, la colonne fantôme déjà documentée partout ailleurs dans ce dépôt) — alors que `$tenantId` reçu par cette méthode est en réalité un `company_id`. Les ratios `inventory_turnover`/`stockout_rate` retombaient donc systématiquement sur leur valeur de repli statique. Corrigé vers la vraie colonne `company_id`. **Point de vigilance pour la fusion** : ce fichier est très activement modifié en parallèle par un audit Strategy concurrent (Chantier 32.27), dont le docblock affirme à tort que `inventory_products.tenant_id` est « déjà alimentée depuis company_id par le chemin d'écriture du module » — généralisation correcte pour Sales/CRM/HR/Helpdesk mais **empiriquement fausse pour Inventory**, reconfirmée juste avant cette rédaction (`ProductController.php:111`).
- **Layer 13 (IA)** : seules 2 des 25 vraies pages Vue du module appelaient `useAiAssistant()`. 4 nouvelles actions de repli fr+en (`view_catalog`, `manage_warehouses`, `manage_suppliers`, `manage_costing_sheets`) et câblage sur 5 pages supplémentaires (`Products/Index.vue` racine, `Warehouses/Index.vue`, `Suppliers/Index.vue` racine, `PurchaseOrders/Index.vue` racine, `CostingSheets/Index.vue`) — 7/25 couvertes, 18 restantes documentées comme un gap de première-vague non totale (même précédent que Chantier 30).
- **Trouvé en même temps (navigation morte)** : les boutons « Import CSV »/« New Product » de `Products/Index.vue` racine n'avaient aucun gestionnaire de clic — câblés vers `/inventory/stock/import`/`/products/create`.
- **Vérifié** : `vendor/bin/pest Modules/Inventory --parallel` → voir CLAUDE.md pour le compte exact ; `php artisan migrate:fresh --seed` propre ; `php -l` propre ; `npm run type-check`/`build` propres.
