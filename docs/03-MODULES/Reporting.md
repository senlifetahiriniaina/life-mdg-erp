...
- **`ReportGenerationService`** — génération des exports PDF/Excel/CSV et création des `ReportExecution`. **Corrigé au Chantier 32.26** : le fallback `tenant_id`/`executed_by` codé en dur à `1` (documenté ci-dessous comme laissé tel quel faute de dispatcher réel) a été supprimé en même temps que le chemin de livraison programmée était réellement activé — voir la section Chantier 32.26 plus bas.
...
- Job asynchrone : `DeliverScheduledReportJob` (livraison programmée par email) — **réellement dispatché depuis le Chantier 32.26** via la nouvelle commande planifiée `reporting:deliver-scheduled`. `RunReportJob` (jamais dispatché nulle part, doublon du chemin synchrone) a été supprimé au même chantier.
...
## Chantier 32.26 — audit approfondi en 14 couches

Voir l'entrée `CLAUDE.md` correspondante pour le détail complet. Résumé :

- **`ReportTemplateSeeder` (les 10 templates OHADA/SYSCOHADA « pre-built ») n'avait jamais été câblé dans `DatabaseSeeder`** — sur toute installation fraîche, les 4 tuiles de rapport rapide de `ReportsIndex.vue` 404aient faute de template seedé. Câblé pour de vrai ; deux bugs de schéma du seeder lui-même corrigés au passage (`category` jamais migrée sur `report_definitions` ; clés `config`/`schedule` fantômes atteignant le vrai SQL parce que `db:seed` tourne sous `Model::unguarded()` — `$fillable` ne filtre jamais pendant un seed).
- **Les 7 templates « `-- Handled by OhadaReportService...` » s'exécutent désormais réellement** : `ReportingService::execute()`/`ReportGenerationService::run()` détectent le `report_type` OHADA et délèguent au vrai `OhadaReportService` au lieu de tenter d'exécuter le commentaire SQL littéral.
- **Les 3 templates SQL réels (#8/#9/#10) réécrits sur le vrai schéma** : ventes (`sales_order_lines.sales_order_id`/`line_total`, `sales_orders.contact_id`/`confirmed_at`, `inventory_products` — pas les colonnes/tables fantômes d'origine, ni les fonctions MySQL-only `DATE_FORMAT()`/`CURDATE()`), stocks (`inventory_products`+`inventory_stock` par `LEFT JOIN`), masse salariale (`payslips`+`hr_employees`+`hr_departments`, statut réel `approved` — `validated` n'a jamais été une vraie valeur).
- **Livraison programmée activée** : nouvelle commande `reporting:deliver-scheduled` (planifiée via `ReportingServiceProvider`), `DeliverScheduledReportJob` corrigé (écrivait `report_definitions.last_run_at`, colonne inexistante ; plantait sur un schedule supprimé) et `RunReportJob` supprimé (mort, jamais dispatché, doublon du chemin synchrone).
- **`ReportGenerationService::run()`** ne code plus `tenant_id`/`executed_by` en dur à `1` et ne produit plus de SQL invalide à double `LIMIT` sur un template portant déjà sa propre clause.
- **`NlToSqlService::queryToSql()`** ne plante plus sur l'introspection de schéma quand une clé Claude est configurée.
- **`ReportPolicy`** réellement appelée depuis les contrôleurs (elle était enregistrée sur le Gate depuis la construction du module mais jamais invoquée).
