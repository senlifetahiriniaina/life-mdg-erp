...
- **7 Accounting controllers remain deliberately unrouted, no matching Vue page for any of them** — wiring them would mean building 7 new full-stack UI features, out of scope for a routing-cleanup pass: `AssetImpairmentController`, `BudgetVarianceController`, `CostEngineController`, `DepreciationPolicyController`, `DepreciationScheduleController`, `IntercompanyClearanceController` (all in `Modules/Accounting/app/Http/Controllers/`, note the `Api/` subfolder inconsistency — 3 of the 6 live directly under `Http/Controllers/`, not `Http/Controllers/Api/`), and `ScenarioPlanningController` (budget scenario simulation/comparison/sensitivity analysis — fully built against the real `acc_budget_scenarios` table, no bugs found, just never routed). Three have real bugs that must be fixed before ever routing them: `AssetImpairmentController` and `DepreciationScheduleController` validate against bare table names (`fixed_assets`, `gl_accounts`, `journal_entries`) instead of the real `acc_`-prefixed tables (`acc_fixed_assets`, `acc_gl_accounts`, `acc_journal_entries`) — every `store()` call would 422; `IntercompanyClearanceController::index()` calls `$q->user()` on a query `Builder` instead of the request, a fatal error if ever hit.
