...
- **GraphQL (Lighthouse) was never wired up** — `config/lighthouse.php:76` points `schema_path` at `project/graphql/schema.graphql`, but neither that file nor the `project/graphql/` directory exists anywhere in this repo. Same "references a never-built feature" profile as the Consolidation/ASC606 gap above — a handful of GraphQL-dependent tests fail on this, not a regression from this extraction. Decide if/when GraphQL ships before building the schema.
