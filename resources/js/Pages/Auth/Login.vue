    ...
    <main class="auth-card">
      ...
      <form @submit.prevent="submit" class="auth-form" novalidate>
        ...
        <div class="form-remember">
          <Checkbox v-model="form.remember" :binary="true" inputId="remember" aria-label="Se souvenir de moi" />
          <label for="remember" class="form-remember-label">Se souvenir de moi</label>
        </div>

        <button type="submit" class="btn-auth" :disabled="form.processing" :aria-busy="form.processing" aria-label="Se connecter">
          <i v-if="form.processing" class="pi pi-spin pi-spinner" style="font-size:14px" aria-hidden="true" />
          <i v-else class="pi pi-sign-in" style="font-size:14px" aria-hidden="true" />
          Se connecter
        </button>
      </form>
    </main>
...
.auth-card {
  ...
  border: 1px solid var(--border-subtle, #D7DCE7);
  border-radius: var(--r-xl, 16px);
  padding: 40px;
  box-shadow: var(--shadow-md, 0 4px 24px rgba(17,20,28,0.08));
}
.auth-logo { text-align: center; margin-bottom: 32px; }
.auth-logo-icon {
  width: 56px; height: 56px;
  background: linear-gradient(135deg, var(--halo-500, #2E5BE8), var(--halo-700, #1A3AA3));
  border-radius: var(--r-lg, 12px);
  display: inline-flex; align-items: center; justify-content: center;
  color: #fff; font-weight: 700; font-size: 22px; font-family: var(--font-display);
  margin-bottom: 16px;
}
.auth-title {
  margin: 0;
...
.form-remember-label { font-size: 13px; color: var(--fg-2); cursor: pointer; }
.btn-auth {
  width: 100%;
  padding: 10px 16px;
