...
const stats = ref({ open_incidents: 0, compliance_score: 0, auth_failures_24h: 0 })
const incidents = ref([])
const threats = ref([])
...
onMounted(async () => {
  try {
    const [s, i, t] = await Promise.all([
      fetch('/api/v1/security/dashboard/summary').then(r => r.json()).catch(() => ({})),
      fetch('/api/v1/security/incidents?status=open&per_page=5').then(r => r.json()).catch(() => ({ data: [] })),
      fetch('/api/v1/security/threat-indicators?limit=5').then(r => r.json()).catch(() => ({ data: [] })),
    ])
    stats.value = { open_incidents: s.open_incidents ?? 0, compliance_score: s.compliance_score ?? 0, auth_failures_24h: s.auth_failures_24h ?? 0 }
  ...
  <div class="p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
        ...
        <p class="text-3xl font-bold text-green-600">{{ stats.compliance_score }}%</p>
      </div>
